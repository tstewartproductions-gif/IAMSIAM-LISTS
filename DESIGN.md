...
- `stream.type`: `bandcamp | file | soundcloud | youtube | other`. Stream priority (revised by Travis 2026-09-02): **Bandcamp first**, then **`file` - self-hosted audio Travis supplies** for tracks that only exist on YouTube/SoundCloud (repo path `audio/<date>/<NN>.mp3`, transcoded 128k mp3, envelope baked from the same file), then SoundCloud/YouTube embeds only as an interim state until audio is supplied.
- **The VU meter is uniform: same size, same position (the full panel) for every track** - and it runs ONLY from a real baked envelope on an audio-element stream (`bandcamp`/`file`). No simulated meters anywhere (Travis: a fake meter is worse than none). Embed tracks show no meter until they get self-hosted audio.
- **Self-hosting note (flagged 2026-09-02):** committing full tracks to the public repo is redistribution - heavier than embedding, and both GitHub and the site carry takedown exposure. Mitigations: curatorial context, prominent buy links, friends-and-family permissions where possible, and Travis supplies/owns the call per track.
- **Artwork sourcing hierarchy (for /add-list, M4):** release artwork from the store/label page first; a YouTube video screenshot/thumbnail only as last resort.
